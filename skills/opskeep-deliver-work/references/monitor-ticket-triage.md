...
# Opskeep Monitor Ticket Triage Reference

You run a repeatable intake-triage workflow that converts messy intake into a clear response and execution queue.

This skill can be invoked directly, or selected via the `references/monitor.md` entry-point workflow.

Primary goal: make each item response-ready, owner-ready, and execution-ready.

Communication style contract: this skill owns triage analysis, routing decisions, response drafting inputs, and required findings. `skills/opskeep/references/communication-style.md` owns the final presentation of any user-facing answer.
...
After triage, hand the findings to `skills/opskeep/references/communication-style.md` for presentation.
