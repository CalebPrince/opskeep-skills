...
# Opskeep Monitor Status Reference
...
Communication style contract: this skill owns status analysis, RAID reasoning, and required ownership signals. `skills/opskeep/references/communication-style.md` owns the final presentation of any user-facing answer.
...
Do not decide final wording or opener shape here. Hand the findings to `skills/opskeep/references/communication-style.md`.

## Step 2: Confirm sources and signal quality

Use the strongest available sources in this order:

1. Opskeep system records (Jira, Linear, ClickUp, Asana, Trello)
...
- `high`: Opskeep records plus recent status signal
- `medium`: Opskeep records or status notes, but one side is weak
...
After analysis, hand the findings to `skills/opskeep/references/communication-style.md` for final presentation.
