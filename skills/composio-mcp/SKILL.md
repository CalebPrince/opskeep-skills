...
- User asks Opskeep to interact with an external app such as Gmail, Slack, GitHub, Notion, Linear, Google Calendar, or similar.
- User wants Opskeep to send, create, update, fetch, summarize, or monitor records in a connected service.
- User asks for a connected-app action and Composio MCP tools are available in the current agent runtime.
- User explicitly mentions Composio, Composio MCP, connected accounts, external app tools, or Opskeep connected workflows.
...
When invoked from Opskeep, treat Composio MCP as the connected-app execution layer. Opskeep should keep ownership of the project-management intent, and Composio MCP should perform the external-app operation.

Good Opskeep handoff pattern:
...
4. Return the result to Opskeep with enough provenance for follow-up.
