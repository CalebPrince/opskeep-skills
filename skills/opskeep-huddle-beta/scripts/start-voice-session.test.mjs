...
test("managed daemon launch stops the previously recorded huddle bridge", async () => {
  const runtimeDir = mkdtempSync(path.join(tmpdir(), "opskeep-voice-session-"));
  const previousBridge = spawn(process.execPath, [scriptPath, "--local"], {
    env: {
      ...process.env,
      OPSKEEP_VOICE_SESSION_RUNTIME_DIR: runtimeDir,
      OPSKEEP_VOICE_SESSION_PORT: String(17_000 + Math.floor(Math.random() * 1_000))
    },
  ...
  const env = {
    OPSKEEP_VOICE_SESSION_RUNTIME_DIR: runtimeDir,
    OPSKEEP_VOICE_SESSION_PORT: String(18_000 + Math.floor(Math.random() * 1_000))
  };
...
test("managed daemon launch does not stop an unrelated process from stale metadata", async () => {
  ...
  const env = {
    OPSKEEP_VOICE_SESSION_RUNTIME_DIR: runtimeDir,
    OPSKEEP_VOICE_SESSION_PORT: String(20_000 + Math.floor(Math.random() * 1_000))
  };
...
test("local bridge exits when the browser session ends", async () => {
  ...
  const child = spawn(process.execPath, [scriptPath, "--local"], {
    ...
    env: {
      ...process.env,
      OPSKEEP_VOICE_SESSION_RUNTIME_DIR: runtimeDir,
      OPSKEEP_VOICE_SESSION_PORT: String(port)
    },
    stdio: ["ignore", "pipe", "pipe"]
  });
...
test("managed daemon exits and clears metadata when the browser session ends", async () => {
  ...
  const env = {
    OPSKEEP_VOICE_SESSION_RUNTIME_DIR: runtimeDir,
    OPSKEEP_VOICE_SESSION_PORT: String(21_000 + Math.floor(Math.random() * 1_000))
  };
  let daemonPid = 0;
...
`);
  ...
  const child = spawn(process.execPath, [scriptPath, "--local"], {
    ...
    env: {
      ...
      OPSKEEP_VOICE_SESSION_CONTEXT: launchContext,
      OPSKEEP_VOICE_SESSION_RUNTIME_DIR: runtimeDir,
      OPSKEEP_VOICE_SESSION_PORT: String(port)
    },
    stdio: ["ignore", "pipe", "pipe"]
  ...
  try {
    ...
    assert.match(prompt, /You are currently in a Opskeep Huddle with the user/);
    assert.match(prompt, /prior conversation context from before the huddle/);
    assert.doesNotMatch(prompt, /You are starting a new Opskeep Huddle voice session/);
