...
const port = Number(process.env.OPSKEEP_VOICE_RELAY_PORT || 8788);
const host = process.env.OPSKEEP_VOICE_RELAY_HOST || "127.0.0.1";
const publicBaseUrl = (process.env.OPSKEEP_VOICE_RELAY_PUBLIC_BASE_URL || `http://${host}:${port}`).replace(/\/$/, "");
const allowedOrigins = new Set((process.env.OPSKEEP_VOICE_RELAY_ALLOWED_ORIGINS || publicBaseUrl).split(",").map((item) => item.trim()).filter(Boolean));
const disabled = /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_RELAY_DISABLED || "");
const printSession = /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_RELAY_PRINT_SESSION || "") || /^https?:\/\/(127\.0\.0\.1|localhost)(:|\/|$)/.test(publicBaseUrl);
const allowQueryToken = /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_RELAY_ALLOW_QUERY_TOKEN || "");
const operatorToken = process.env.OPSKEEP_VOICE_RELAY_OPERATOR_TOKEN || "";
const maxMessageBytes = Number(process.env.OPSKEEP_VOICE_RELAY_MAX_MESSAGE_BYTES || 32_000);
const pairingTtlMs = Number(process.env.OPSKEEP_VOICE_RELAY_PAIRING_TTL_SECONDS || 900) * 1000;
const idleTimeoutMs = Number(process.env.OPSKEEP_VOICE_RELAY_IDLE_TIMEOUT_SECONDS || 600) * 1000;
const reconnectGraceMs = Number(process.env.OPSKEEP_VOICE_RELAY_RECONNECT_GRACE_SECONDS || 15) * 1000;
const maxSessions = Number(process.env.OPSKEEP_VOICE_RELAY_MAX_SESSIONS || 100);
const maxInvalidFrames = Number(process.env.OPSKEEP_VOICE_RELAY_MAX_INVALID_FRAMES || 3);
const maxMessagesPerWindow = Number(process.env.OPSKEEP_VOICE_RELAY_MAX_MESSAGES_PER_WINDOW || 30);
const rateWindowMs = Number(process.env.OPSKEEP_VOICE_RELAY_RATE_WINDOW_SECONDS || 10) * 1000;
...
function authFromUpgrade(req, url) {
  ...
  return {
    role: protocolValue(protocols, "opskeep-role.") || (allowQueryToken ? url.searchParams.get("role") : ""),
    sessionId: protocolValue(protocols, "opskeep-session.") || (allowQueryToken ? url.searchParams.get("session_id") : ""),
    token: protocolValue(protocols, "opskeep-token.") || (allowQueryToken ? url.searchParams.get("token") : "")
  };
}

function devIndexHtml() {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Opskeep Huddle Dev Relay</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;max-width:680px;margin:64px auto;padding:0 24px;line-height:1.6;color:#2d2a25;background:#fbfaf7}.muted{color:#706b61}code{background:#efede7;border-radius:6px;padding:2px 5px}</style></head><body><p class="muted">Development relay</p><h1>Opskeep Huddle relay is running.</h1><p>Create a session with <code>POST /api/sessions</code>, then open the returned <code>/s/&lt;session-id&gt;#token=...</code> browser URL.</p></body></html>`;
}
...
function hasOperatorAccess(req) {
  ...
  const headerToken = String(req.headers["x-opskeep-relay-operator-token"] || "");
...
const wss = new WebSocketServer({ noServer: true, perMessageDeflate: false, maxPayload: maxMessageBytes, handleProtocols: () => "opskeep-relay" });
...
server.listen(port, host, () => {
  console.log(`Opskeep hosted relay: http://${host}:${port}`);
  ...
  if (initial.ok && printSession) {
    ...
    console.log(`OPSKEEP_VOICE_RELAY_URL=${publicBaseUrl.replace(/^http/, "ws")}/ws`);
    console.log(`OPSKEEP_VOICE_RELAY_SESSION_ID=${initial.package.session_id}`);
    console.log(`OPSKEEP_VOICE_RELAY_BRIDGE_TOKEN=${initial.package.bridge.token}`);
    console.log("OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS=<optional supported opencode run args>");
  } else if (initial.ok) {
