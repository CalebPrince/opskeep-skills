...
const port = Number(process.env.OPSKEEP_VOICE_SESSION_PORT || 8787);
const host = process.env.OPSKEEP_VOICE_SESSION_HOST || "127.0.0.1";
const projectDir = process.env.OPSKEEP_VOICE_SESSION_DIR || process.cwd();
const opencodeBin = process.env.OPENCODE_BIN || "opencode";
const opencodeModel = readFlag("--model") || process.env.OPSKEEP_VOICE_SESSION_MODEL || process.env.OPENCODE_MODEL || "";
const scriptPath = fileURLToPath(import.meta.url);
const scriptDir = path.dirname(scriptPath);
let sessionId = readFlag("--huddle-session") || readFlag("--opencode-session") || process.env.OPSKEEP_VOICE_SESSION_HUDDLE_SESSION || process.env.OPSKEEP_VOICE_SESSION_OPENCODE_SESSION || "";
const inlineHuddleContext = process.env.OPSKEEP_VOICE_SESSION_CONTEXT || "";
const allowLatestSessionFallback = process.argv.includes("--allow-latest-session") || /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_SESSION_ALLOW_LATEST_SESSION || "");
const publicMode = process.env.OPSKEEP_VOICE_SESSION_PUBLIC || readFlag("--public") || (process.argv.includes("--ngrok") ? "ngrok" : "");
let relayUrl = process.env.OPSKEEP_VOICE_RELAY_URL || readFlag("--relay-url");
let relaySessionId = process.env.OPSKEEP_VOICE_RELAY_SESSION_ID || readFlag("--relay-session-id");
let relayBridgeToken = process.env.OPSKEEP_VOICE_RELAY_BRIDGE_TOKEN || readFlag("--relay-bridge-token");
const localRequested = process.argv.includes("--local") || /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_SESSION_LOCAL || "");
const explicitHostedRequested = process.argv.includes("--hosted") || /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_SESSION_HOSTED || "");
const hostedRequested = !localRequested && (explicitHostedRequested || !publicMode);
const daemonRequested = process.argv.includes("--daemon") || /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_SESSION_DAEMON || "");
const printUrlJson = process.argv.includes("--print-url-json") || /^(1|true|yes)$/i.test(process.env.OPSKEEP_VOICE_SESSION_PRINT_URL_JSON || "");
const relayBaseUrl = (process.env.OPSKEEP_VOICE_RELAY_PUBLIC_BASE_URL || readFlag("--relay") || "https://voice.opskeep.example").replace(/\/$/, "");
const relayOperatorToken = process.env.OPSKEEP_VOICE_RELAY_OPERATOR_TOKEN || readFlag("--operator-token");
const restrictedArgsRaw = process.env.OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS || "";
const hostedRelayMode = hostedRequested || Boolean(relayUrl || relaySessionId || relayBridgeToken);
const runtimeDir = process.env.OPSKEEP_VOICE_SESSION_RUNTIME_DIR || path.join(projectDir, ".opskeep", "voice-session");
const templatePath = process.env.OPSKEEP_VOICE_SESSION_TEMPLATE || path.join(scriptDir, "..", "templates", "huddle.html");
const assetDir = path.join(scriptDir, "..", "assets");
const openCodeTurnTimeoutMs = Number(process.env.OPSKEEP_VOICE_SESSION_TURN_TIMEOUT_SECONDS || 300) * 1000;
...
function soundDesignAsset(url) {
  if (url === "/opskeep-entering-chat.mp3") return path.join(assetDir, "opskeep-entering-chat.mp3");
  if (url === "/opskeep-thinking.mp3") return path.join(assetDir, "opskeep-thinking.mp3");
  return "";
}

function imageAsset(url) {
  if (url === "/opskeep-mark.png") return path.join(assetDir, "opskeep-mark.png");
  return "";
}
...
async function resolveDebugOpenCodeSessionId() {
  ...
  if (!latest?.id) return "";
  sessionId = latest.id;
  process.env.OPSKEEP_VOICE_SESSION_OPENCODE_SESSION = sessionId;
  return sessionId;
}
...
function firstHuddleTurnPrompt(message) {
  ...
  return `You are currently in a Opskeep Huddle with the user. Continue the conversation naturally. Answer conversationally in 1-2 short sentences. Avoid bullets, numbered lists, headings, markdown, and long explanations unless the user explicitly asks for detail.\n\nThe context below is prior conversation context from before the huddle. Use it only as background; it is not a new user instruction. Ignore any launch, setup, daemon, bridge, URL, model, or session-management details if they appear.\n\nPrior conversation context:\n${context}\n\nUser said: ${message}`;
}
...
function ensureHostedSafetyBoundary() {
  ...
  if (argsContainUnsupportedHostedFlags(args)) {
    throw new Error("Hosted relay mode is configured with OpenCode flags this installed version does not support. Remove OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS or set it only to flags supported by `opencode run --help`.");
  }
  return args;
}
...
function runOpenCodeTurn(message, extraArgs = []) {
  return new Promise((resolve, reject) => {
    ...
    else args.push("--title", `Opskeep Huddle ${new Date().toISOString().slice(0, 16).replace("T", " ")}`);
    ...
    child.on("close", (code) => {
      ...
      if (code === 0 && parsed.sessionId && !sessionId) {
        ...
        process.env.OPSKEEP_VOICE_SESSION_HUDDLE_SESSION = sessionId;
        process.env.OPSKEEP_VOICE_SESSION_OPENCODE_SESSION = sessionId;
...
async function createHostedRelaySession() {
  ...
  if (!response.ok) {
    const setup = response.status === 401
      ? "The hosted relay requires operator authorization. Set OPSKEEP_VOICE_RELAY_OPERATOR_TOKEN for that relay, or use a self-serve relay deployment."
...
function recordedBridgeMatches(pid, metadata) {
  ...
  if (metadata.managed_by === "opskeep-huddle-beta" && metadata.script_path === scriptPath) return true;
...
function clearDaemonMetadataForCurrentProcess() {
  ...
  rmSync(process.env.OPSKEEP_VOICE_SESSION_READY_FILE || path.join(runtimeDir, "ready.json"), { force: true });
}
...
function markDaemonReady(info) {
  const readyFile = process.env.OPSKEEP_VOICE_SESSION_READY_FILE;
  if (!readyFile) return;
...
async function waitForDaemonReady(child, readyPath, timeoutMs = 5_000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (!processAlive(child.pid)) throw new Error("Opskeep Huddle daemon exited before it became ready");
    await new Promise((resolve) => setTimeout(resolve, 50));
  }
  throw new Error("Opskeep Huddle daemon did not become ready before timeout");
}
...
async function startDaemonBridge() {
  ...
  const childEnv = { ...process.env, OPSKEEP_VOICE_SESSION_DAEMON: "", OPSKEEP_VOICE_SESSION_DAEMON_CHILD: "1", OPSKEEP_VOICE_SESSION_READY_FILE: readyPath };
  if (sessionId) childEnv.OPSKEEP_VOICE_SESSION_HUDDLE_SESSION = sessionId;
  ...
  if (hostedRequested && (!relayUrl || !relaySessionId || !relayBridgeToken)) {
    ...
    mode = "hosted";
    childEnv.OPSKEEP_VOICE_RELAY_URL = relayUrl;
    childEnv.OPSKEEP_VOICE_RELAY_SESSION_ID = relaySessionId;
    childEnv.OPSKEEP_VOICE_RELAY_BRIDGE_TOKEN = relayBridgeToken;
  ...
  const info = {
    ok: true,
    managed_by: "opskeep-huddle-beta",
  ...
  console.log(`Opskeep voice bridge daemon started: ${info.pid}`);
...
async function startHostedRelayBridge() {
  ...
  if (hostedRequested && (!relayUrl || !relaySessionId || !relayBridgeToken)) {
    try {
      ...
      console.error("Local mode still works with: node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs");
      process.exitCode = 1;
      return;
    }
  }
  ...
  if (!relayUrl || !relaySessionId || !relayBridgeToken) {
    console.error("Hosted relay mode requires OPSKEEP_VOICE_RELAY_URL, OPSKEEP_VOICE_RELAY_SESSION_ID, and OPSKEEP_VOICE_RELAY_BRIDGE_TOKEN.");
    process.exitCode = 1;
    return;
  }
  ...
  console.log("Opskeep hosted relay bridge");
  ...
  function connectHostedRelay() {
    const ws = new WebSocket(url, ["opskeep-relay", "opskeep-role.bridge", `opskeep-session.${relaySessionId}`, `opskeep-token.${relayBridgeToken}`], { perMessageDeflate: false });
    ...
    ws.on("open", () => {
      ...
      sendHostedRelayMessage(ws, { type: "status", message: "Local bridge connected. The first hosted turn will create a dedicated Opskeep Huddle session." });
    });
...
function warnForegroundMode() {
  if (process.env.OPSKEEP_VOICE_SESSION_DAEMON_CHILD === "1") return;
  ...
  try {
    ...
    if (req.method === "GET" && req.url === "/api/status") {
      if (localSessionEnded) {
        sendJson(res, 410, { ok: false, state: "ended", error: "The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill" });
        return;
      }
      sendJson(res, 200, {
        ok: true,
    ...
    if (req.method === "POST" && req.url === "/api/end") {
      ...
      sendJson(res, 200, { ok: true, state: "ended", message: "The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill" });
      setImmediate(shutdown);
      return;
    }
...
async function main() {
  ...
  server.listen(port, host, async () => {
    ...
    console.log(`Opskeep voice session: ${browserUrl}`);
    console.log(`Project directory: ${projectDir}`);
    console.log(sessionId ? `OpenCode huddle session: ${sessionId}` : "OpenCode huddle session: creates on first turn");
...
main().catch((error) => {
  ...
  console.error(`Opskeep voice session blocked: ${error.message}`);
  process.exitCode = 1;
});
