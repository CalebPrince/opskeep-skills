...
function hostedAdapterScript() {
  return String.raw`<script>
      const sessionId = __OPSKEEP_HOSTED_SESSION_ID__;
      const token = new URLSearchParams(location.hash.slice(1)).get("token");
      const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
      const wsUrl = (location.protocol === "https:" ? "wss://" : "ws://") + location.host + "/ws/" + encodeURIComponent(sessionId);
      const labels = { ready:"Ready", listening:"Listening", processing:"Thinking", speaking:"Speaking", paused:"Paused", muted:"Muted", reconnecting:"Reconnecting", disconnected:"Disconnected", blocked:"Ready" };
      ...
      const state = { ws:null, paired:false, busy:false, listening:false, muted:false, paused:false, ended:false, recognition:null, finalTranscript:"", interimTranscript:"", silenceTimer:null, maxListenTimer:null, stopRequested:false, thinkingAudio:null, turns:[], turnSeq:0, reconnectTimer:null, reconnectStartedAt:0, reconnectAttempts:0, reconnectGraceMs:15000 };
      const sounds = { entering:"__OPSKEEP_ENTRY_SOUND__", thinking:"__OPSKEEP_THINKING_SOUND__" };
      ...
      function playSound(src, options = {}) { const audio = new Audio(src); audio.loop = Boolean(options.loop); audio.volume = options.volume ?? 0.55; const started = audio.play(); if (started && started.catch) started.catch(() => {}); return audio; }
      ...
      function send(message) { if (state.ws && state.ws.readyState === WebSocket.OPEN) state.ws.send(JSON.stringify(message)); }
      function submitTurn(text) { const clean = text.trim(); if (!clean || state.busy || state.ended) return; if (!state.paired) { setState(state.reconnectStartedAt ? "reconnecting" : "paused"); setLiveTranscript(state.reconnectStartedAt ? "Reconnecting to the hosted relay..." : "Waiting for the local OpenCode bridge to connect...", { kind:"interim" }); return; } state.busy = true; addTurn("user", clean); setState("processing"); setLiveTranscript("Thinking", { kind:"thinking" }); startThinkingSound(); send({ type:"user_turn", turn_id:"browser-" + Date.now() + "-" + (++state.turnSeq), text:clean }); }
      function stopListening(shouldSubmit) { const text = currentTranscript(); state.stopRequested = true; clearSpeechTimers(); try { if (state.recognition) state.recognition.stop(); } catch (_error) {} state.recognition = null; state.listening = false; state.finalTranscript = ""; state.interimTranscript = ""; if (shouldSubmit && text) submitTurn(text); else if (!state.busy) { setState("ready"); setLiveTranscript(""); } }
      ...
      function resetReconnect() { clearReconnectTimer(); state.reconnectStartedAt = 0; state.reconnectAttempts = 0; }
      function markDisconnected(message) { state.ended = true; state.paired = false; resetReconnect(); stopThinkingSound(); clearSpeechTimers(); try { if (state.recognition) state.recognition.stop(); } catch (_error) {} state.recognition = null; state.listening = false; setState("disconnected"); setLiveTranscript(message || "The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill", { kind:"interim" }); }
      function scheduleReconnect() { if (state.ended) return; state.paired = false; clearSpeechTimers(); try { if (state.recognition) state.recognition.stop(); } catch (_error) {} state.recognition = null; state.listening = false; const now = Date.now(); if (!state.reconnectStartedAt) state.reconnectStartedAt = now; const elapsed = now - state.reconnectStartedAt; if (elapsed >= state.reconnectGraceMs) { markDisconnected("The hosted relay connection could not be restored. Ask your agent to reconnect using the opskeep-huddle-beta skill"); return; } setState("reconnecting"); setLiveTranscript("Connection interrupted. Reconnecting to the hosted relay...", { kind:"interim" }); if (state.reconnectTimer) return; const remaining = state.reconnectGraceMs - elapsed; const delay = Math.min(1000 * Math.max(1, 2 ** state.reconnectAttempts), 4000, remaining); state.reconnectTimer = setTimeout(() => { state.reconnectTimer = null; state.reconnectAttempts += 1; connect(); }, delay); }
      function handleStatus(message) { state.paired = message.state === "paired" || message.state === "active_turn"; if (state.paired) { resetReconnect(); if (!state.busy && !state.paused && !state.muted) { setState("ready"); setLiveTranscript(""); } return; } if (message.state === "degraded") { setState("reconnecting"); setLiveTranscript("Reconnecting. Waiting for the hosted relay to restore both sides...", { kind:"interim" }); return; } if (message.state === "expired" || message.state === "ended") { markDisconnected("The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill"); return; } if (!state.reconnectStartedAt && !state.busy) { setState("paused"); setLiveTranscript(message.message || "Waiting for the local OpenCode bridge to connect...", { kind:"interim" }); } }
      function connect() { if (!token) { markDisconnected("This link is missing session credentials. Ask your agent to follow the opskeep-huddle-beta skill for a new URL."); return; } const ws = new WebSocket(wsUrl, ["opskeep-relay", "opskeep-role.browser", "opskeep-session." + sessionId, "opskeep-token." + token]); state.ws = ws; ws.onopen = () => { if (!state.paired) setLiveTranscript(state.reconnectStartedAt ? "Reconnected. Waiting for the local OpenCode bridge..." : "Waiting for the local OpenCode bridge to connect...", { kind:"interim" }); }; ws.onmessage = (event) => { const message = JSON.parse(event.data); if (message.type === "status") handleStatus(message); if (message.type === "assistant_reply") { resetReconnect(); stopThinkingSound(); state.busy = false; addTurn("agent", message.text); setState("speaking"); setLiveTranscript(message.text, { kind:"agent" }); speak(message.text, () => { if (state.paired && !state.busy && !state.ended && !state.paused && !state.muted) startVoiceTurn({ automatic:true }); }); } if (message.type === "error") { stopThinkingSound(); state.busy = false; setState("ready"); setLiveTranscript("Bridge blocked: " + message.message, { kind:"interim" }); addTurn("System", "Blocked: " + message.message); } if (message.type === "end") markDisconnected("The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill"); }; ws.onclose = () => { if (state.ws === ws) state.ws = null; if (!state.ended) scheduleReconnect(); }; ws.onerror = () => { if (!state.ended) setLiveTranscript("Connection interrupted. Reconnecting to the hosted relay...", { kind:"interim" }); }; }
      ...
      document.querySelectorAll("[data-end-session]").forEach((btn) => btn.addEventListener("click", () => { state.ended = true; try { if (state.ws) state.ws.send(JSON.stringify({ type:"end" })); } catch (_error) {} try { if (state.ws) state.ws.close(1000, "ended"); } catch (_error) {} window.speechSynthesis?.cancel(); stopListening(false); markDisconnected("The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill"); }));
...
function buildHostedHtml() {
  ...
  return template
    .slice(0, scriptStart)
    .replaceAll("/opskeep-mark.png", opskeepMark)
    + hostedAdapterScript()
      .replaceAll("__OPSKEEP_ENTRY_SOUND__", entrySound)
      .replaceAll("__OPSKEEP_THINKING_SOUND__", thinkingSound)
    + template.slice(scriptEnd + "</script>".length);
}

const opskeepMark = dataUri("opskeep-mark.png", "image/png");
const entrySound = dataUri("opskeep-entering-chat.mp3", "audio/mpeg");
const thinkingSound = dataUri("opskeep-thinking.mp3", "audio/mpeg");
const hostedHtml = buildHostedHtml();
const output = `// Generated by scripts/sync-hosted-template.mjs. Do not edit by hand.\n\nexport function hostedSessionHtml(sessionId) {\n  const safeSessionId = JSON.stringify(String(sessionId || "").replace(/[<>&"']/g, ""));\n  return ${JSON.stringify(hostedHtml)}.replace("__OPSKEEP_HOSTED_SESSION_ID__", safeSessionId);\n}\n`;
