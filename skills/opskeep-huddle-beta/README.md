...
If you copy or install only the `skills/opskeep-huddle-beta` directory, install its local Node dependency first:

```bash
npm install --prefix skills/opskeep-huddle-beta
```

Hosted mode is the default. The command creates a per-huddle hosted session at `voice.opskeep.example`, starts the local bridge as a managed daemon, connects it to your current OpenCode context, and prints JSON events with the browser link and process metadata:

```json
{"event":"voice_session_daemon_started","browser_url":"https://voice.opskeep.example/s/<session-id>#token=...","pid":12345,"stop_command":"kill 12345","log_path":".opskeep/voice-session/bridge.log"}
```

Open that link in any browser with microphone access. Each run gets its own session URL and credentials, so multiple users can use `voice.opskeep.example` at the same time without sharing a room.

The process metadata is for the launching agent or operator. Each managed launch stops only the previously recorded project-local Opskeep Huddle bridge from `.opskeep/voice-session/bridge.pid`, writes fresh metadata to `.opskeep/voice-session/session.json`, and exits when the huddle session ends.
...
To seed the huddle with prior conversation context, pass a compact summary inline with `OPSKEEP_VOICE_SESSION_CONTEXT` for that launch only. The summary should only include what the user was discussing, relevant decisions/preferences, open questions, and files or repo details needed to understand that discussion. If the user only asked to start a huddle, omit context. Do not include launch mechanics, daemon details, URLs, model/runtime details, or instructions to start the huddle; the huddle has already started by the time this context is used.

Pass `--model <current-opencode-model>` from the launching OpenCode session so the huddle does not fall back to a different machine default provider. The bridge also honors `OPSKEEP_VOICE_SESSION_MODEL` and `OPENCODE_MODEL`, but explicit `--model` is the clearest launch path.

## What It Does

- renders the deterministic Opskeep Huddle template from `templates/huddle.html`
...
```bash
OPSKEEP_VOICE_SESSION_TEMPLATE=/absolute/path/to/index.html node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs --daemon --print-url-json --local
```
...
OpenCode turns default to a 300-second timeout so larger planning or implementation requests have room to finish. Override with `OPSKEEP_VOICE_SESSION_TURN_TIMEOUT_SECONDS=<seconds>` when testing unusually long turns.
