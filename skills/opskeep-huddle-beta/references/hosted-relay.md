...
Each hosted session gets its own generated session id and role-separated credentials. Browser links use `https://voice.opskeep.example/s/<session-id>#token=...`. The browser token stays in the URL fragment and the bridge token is printed only to the local bridge process.
...
Hosted mode does not add OpenCode flags by default and does not pass `--dangerously-skip-permissions`. It forwards spoken/text turns to `opencode run`; OpenCode's normal session and permission behavior still applies. The bridge creates a dedicated Opskeep Huddle OpenCode session on the first turn and then uses `--session <id>` for later turns, so voice work stays in the huddle side-room instead of following whatever later becomes the last session. Custom `OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS` must use flags supported by the installed OpenCode version; unsupported safety flags such as `--no-tools` are not passed through.
...
For sandboxed or remote-browser voice sessions, use the Opskeep hosted relay path:

```bash
node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs --daemon --print-url-json --model <current-opencode-model>
```
...
Each managed launch stops only the previously recorded project-local Opskeep Huddle bridge from `.opskeep/voice-session/bridge.pid`, writes fresh metadata to `.opskeep/voice-session/session.json`, and exits when the huddle session ends.
...
- `cloudflare/wrangler.jsonc`: `voice.opskeep.example/*` route, Durable Object binding, migration, and production vars
...
- `OPSKEEP_VOICE_RELAY_PUBLIC_BASE_URL`: public HTTPS origin for browser links
- `OPSKEEP_VOICE_RELAY_ALLOWED_ORIGINS`: comma-separated allowed browser origins
- `OPSKEEP_VOICE_RELAY_PORT`: HTTP server port, default `8788`
- `OPSKEEP_VOICE_RELAY_PAIRING_TTL_SECONDS`: time to wait for both roles to join
- `OPSKEEP_VOICE_RELAY_IDLE_TIMEOUT_SECONDS`: idle close timeout, default 10 minutes
- `OPSKEEP_VOICE_RELAY_RECONNECT_GRACE_SECONDS`: local dev relay reconnect window after a paired role disconnects, default 15 seconds
- hosted Worker `RECONNECT_GRACE_SECONDS`: reconnect window after a paired role disconnects, default 15 seconds
- `OPSKEEP_VOICE_RELAY_MAX_MESSAGE_BYTES`: maximum text frame size
- `OPSKEEP_VOICE_RELAY_MAX_SESSIONS`: max in-memory concurrent sessions
- `OPSKEEP_VOICE_RELAY_DISABLED`: kill switch; when truthy, new session creation is blocked
- `OPSKEEP_VOICE_RELAY_PRINT_SESSION`: local-dev convenience only; prints initial session credentials when explicitly enabled
- `OPSKEEP_VOICE_RELAY_OPERATOR_TOKEN`: optional bearer or `x-opskeep-relay-operator-token` credential for `POST /api/sessions`; when unset, session creation is self-serve and protected by capability-token pairing
- `OPSKEEP_VOICE_RELAY_ALLOW_QUERY_TOKEN`: local/debug-only compatibility switch for query-string WebSocket credentials
...
- `OPSKEEP_VOICE_RELAY_URL`: hosted relay WebSocket URL
- `OPSKEEP_VOICE_RELAY_SESSION_ID`: session id returned by the relay
- `OPSKEEP_VOICE_RELAY_BRIDGE_TOKEN`: bridge role credential
- `OPSKEEP_VOICE_SESSION_HUDDLE_SESSION`: optional existing huddle session id to resume
- `OPSKEEP_VOICE_SESSION_OPENCODE_SESSION`: backward-compatible alias for `OPSKEEP_VOICE_SESSION_HUDDLE_SESSION`
- `OPSKEEP_VOICE_SESSION_CONTEXT`: optional inline prior conversation context for this launch only, injected into the first huddle turn only
...
- `OPSKEEP_VOICE_SESSION_MODEL`: environment equivalent to `--model`, for automation only
- `--local` or `OPSKEEP_VOICE_SESSION_LOCAL=1`: force the same-computer local bridge instead of the default hosted relay
- `OPSKEEP_VOICE_SESSION_ALLOW_LATEST_SESSION`: manual-debug fallback only; allows pinning the latest session when intentionally debugging without a huddle session id
- `OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS`: optional OpenCode args for hosted mode; unset by default
...
- if operator-authorized, keep `OPSKEEP_VOICE_RELAY_OPERATOR_TOKEN` in a managed secret store
...
Production should not use a Cloudflare Tunnel to a developer machine. Deploy the Worker with `npm run deploy:voice-relay` after `npx wrangler login`; the route `voice.opskeep.example/*` should point at the Worker. Production monitoring, accounts, and team access controls are follow-up work.
...
- **Next:** restore or deploy the Worker at `voice.opskeep.example`, or use local mode until hosted relay is available
```
