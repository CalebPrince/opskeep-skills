...
```bash
node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs --daemon --print-url-json --local --model <current-opencode-model>
```

Open the `browser_url` from the JSON output. Managed launches stop only the previously recorded project-local Opskeep Huddle bridge from `.opskeep/voice-session/bridge.pid`, write fresh metadata to `.opskeep/voice-session/session.json`, and exit when the huddle session ends.

The bridge does this for each user turn:

```text
Opskeep Huddle template -> Browser SpeechRecognition -> POST /api/turn -> first turn creates Opskeep Huddle session -> later turns use opencode run --session <huddle-session-id> --dir <repo> -> browser SpeechSynthesis
```

Environment variables:

- `OPSKEEP_VOICE_SESSION_PORT`: defaults to `8787`
- `OPSKEEP_VOICE_SESSION_HOST`: defaults to `127.0.0.1`
- `OPSKEEP_VOICE_SESSION_DIR`: defaults to the server working directory
- `OPSKEEP_VOICE_SESSION_HUDDLE_SESSION`: optional existing huddle session id to resume; otherwise the bridge creates a fresh huddle session on the first turn
...
- `OPSKEEP_VOICE_SESSION_MODEL`: environment equivalent to `--model <current-opencode-model>` for automation
- `OPENCODE_MODEL`: fallback model source when set by the runtime; explicit `--model` remains preferred for inspectable launches
- `OPSKEEP_VOICE_SESSION_ALLOW_LATEST_SESSION`: manual-debug fallback only; set to `1` or pass `--allow-latest-session` to pin the latest `opencode session list --format json --max-count 1` result when intentionally debugging without a huddle session id
- `--daemon --print-url-json`: standard launch mode for managed bridge lifetime and machine-readable browser URL output
- `OPSKEEP_VOICE_SESSION_TEMPLATE`: optional absolute HTML template path; defaults to the skill-local `templates/huddle.html`
- `OPSKEEP_VOICE_SESSION_TURN_TIMEOUT_SECONDS`: OpenCode turn timeout, defaults to `300`
- `OPENCODE_BIN`: defaults to `opencode`

Hosted relay bridge mode environment variables:

- Normal user path: `node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs --daemon --print-url-json --model <current-opencode-model>`
- `OPSKEEP_VOICE_RELAY_PUBLIC_BASE_URL`: optional override for session creation; defaults to `https://voice.opskeep.example`
- `OPSKEEP_VOICE_RELAY_URL`: operator/debug-only hosted relay WebSocket URL
- `OPSKEEP_VOICE_RELAY_SESSION_ID`: operator/debug-only hosted relay session id
- `OPSKEEP_VOICE_RELAY_BRIDGE_TOKEN`: operator/debug-only bridge role token for the session
- `OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS`: optional hosted-mode OpenCode args; unset by default and must use flags supported by the installed `opencode run`
...
- serve the bundled UI through `node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs --daemon --print-url-json`
- keep the UI deterministic through `references/template-contract.md`
- call `opencode run --format json --title "Opskeep Huddle" --dir <repo>` for the first turn when no huddle session exists, plus `--model <current-opencode-model>` from the launching session
...
Hosted relay mode routes remote browser text into the local bridge and is the default managed launch when `--local` is not passed. Hosted mode does not add OpenCode flags by default. If custom `OPSKEEP_VOICE_SESSION_OPENCODE_RESTRICTED_ARGS` are set, they must be supported by the installed `opencode run` or the bridge blocks instead of invoking OpenCode.
