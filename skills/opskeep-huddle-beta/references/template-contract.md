# Opskeep Huddle Template Contract
...
```text
skills/opskeep-huddle-beta/templates/huddle.html
```

This file is part of the skill package so someone can download `skills/opskeep-huddle-beta/` by itself and get the same huddle UI. Prototype folders may mirror or experiment with the template, but they are not the source of truth.

All template assets must also live inside the skill package. Current assets are stored under:

```text
skills/opskeep-huddle-beta/assets/
```
...
The template must be a static HTML file with no build step. It must load over localhost, ngrok HTTPS, or the hosted relay page without relying on bundlers, prototype folders, or private assets outside `skills/opskeep-huddle-beta/`.
...
- light theme only
- `Opskeep Huddle` brand with the skill-local `assets/opskeep-mark.png` logo
...
- entry and thinking sound cues served by the local bridge from skill-local `assets/opskeep-entering-chat.mp3` and `assets/opskeep-thinking.mp3`
...
When the user ends the session or the bridge/relay becomes unavailable, the UI should stay in the huddle and show this disconnected state copy: `The session is disconnected. To connect a new session, ask your agent to reconnect using the opskeep-huddle-beta skill`
