...
Communication style contract: when returning user-facing status, blockers, or final handoffs, apply `skills/opskeep/references/communication-style.md` when available.
...
Use this skill when the user asks to talk with Opskeep or the active agent by voice, including hosted or relay-backed sessions from another browser. Route audio artifacts to `opskeep-audio-brief`, human video meetings elsewhere, and realtime voice coding or spoken approvals to a clear scope blocker.
...
When context is useful, pass it inline with `OPSKEEP_VOICE_SESSION_CONTEXT`. The bridge injects this context into the first huddle turn only, so the dedicated huddle session starts informed without continuing the caller's thread. Do not create or update launch context files as part of the normal workflow.
...
Use the managed hosted relay path for sandboxed or remote-browser use. The production relay is `https://voice.opskeep.example`; each launch creates a separate session URL under `/s/<session-id>` and pairs it with a local bridge daemon that connects outbound to `/ws/<session-id>`.

Use the bundled same-computer bridge at `skills/opskeep-huddle-beta/scripts/start-voice-session.mjs` as the local fallback and development path. It serves a localhost browser UI, captures speech with browser APIs, sends each turn to `opencode run`, and speaks the OpenCode response with browser speech synthesis.

Always launch through the managed daemon JSON path. The bridge is a long-running HTTP/WebSocket process, so it must not be coupled to the shell or tool-call lifetime. A new managed launch stops only the previously recorded project-local Opskeep Huddle bridge from `.opskeep/voice-session/bridge.pid`, writes fresh metadata to `.opskeep/voice-session/session.json`, and exits when the huddle session ends.

The local bridge serves the deterministic huddle template defined in `references/template-contract.md`. By default it renders the skill-local file `templates/huddle.html`; set `OPSKEEP_VOICE_SESSION_TEMPLATE=/absolute/path/to/index.html` only for explicit template experiments.
...
```bash
OPSKEEP_VOICE_SESSION_CONTEXT='<prior conversation context>' node skills/opskeep-huddle-beta/scripts/start-voice-session.mjs --daemon --print-url-json --model <current-opencode-model>
```
