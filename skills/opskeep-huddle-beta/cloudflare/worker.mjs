...
export class VoiceSession {
  ...
  async fetch(request) {
    ...
    return new Response(null, { status: 101, webSocket: client, headers: { "Sec-WebSocket-Protocol": "opskeep-relay" } });
  }
...
function authFromProtocols(header) {
  ...
  return { role: value("opskeep-role."), sessionId: value("opskeep-session."), token: value("opskeep-token.") };
}
...
<html lang="en">
  ...
  <body>
    ...
    <h1>Opskeep Huddle</h1>
    <p class="lede">Opskeep Huddle lets you talk through work out loud with an agent. Right now it works with Opskeep or OpenCode by pairing this hosted browser page with a local OpenCode bridge.</p>

    <div class="section">
      <h2>If you use Opskeep</h2>
      <p>Ask Opskeep to start a huddle. Opskeep will route to the <code class="inline-code">opskeep-huddle-beta</code> skill, create a short-lived session link, and connect the local bridge for you.</p>
    </div>

    <div class="section">
      <h2>If you want to try it with OpenCode</h2>
      <p>Paste this into your OpenCode agent:</p>
      <code class="command">Install the Opskeep Huddle Beta skill with:

npx skills add CalebPrince/opskeep-skills --skill opskeep-huddle-beta

Then follow the instructions in skills/opskeep-huddle-beta/SKILL.md to start a hosted huddle.</code>
      <ol>
