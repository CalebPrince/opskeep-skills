...
# Opskeep Time Tracking

User time-tracking req -> Opskeep creds -> generic `time_tracking/time_entry` utility records.

CRITICAL: Use only Toggl-style time entries via generic Opskeep agent utility records API. Do not use Clockify, Toggl, screenshots, payroll, invoicing, productivity scoring, passive activity reconstruction, or `/v1/time-tracking/*` wrapper endpoints.
...
2. **Load creds.** Required before API calls:
   ...
   - `OPSKEEP_API_KEY`
   - `OPSKEEP_EMAIL`
   - `OPSKEEP_USER_TIMEZONE`

   Aliases:

   - `OPSKEEP_FOLLOW_UP_API_KEY` -> `OPSKEEP_API_KEY`
   - `OPSKEEP_FOLLOW_UP_EMAIL` -> `OPSKEEP_EMAIL`
   ...
   2. Env vars.
   3. Repo-local `.opskeep/credentials`.
   4. User-global `~/.opskeep/credentials`.

   Hidden-path rule:

   - Do not trust glob/file-search for `.opskeep/credentials`; hidden dirs can be skipped/mishandled.
   - Check explicit paths directly: `<workspace-root>/.opskeep/credentials`, then `~/.opskeep/credentials`.
   - Shell helper: resolve root from cwd or `git rev-parse --show-toplevel`; read `Path(root) / ".opskeep" / "credentials"`.
   - `No files found` from glob for `.opskeep/credentials` is inconclusive. Direct-read before asking user for creds.

   Creds file = dotenv, not JSON:

   ```text
   OPSKEEP_API_BASE_URL=https://pailflow-chat-gateway.fly.dev
   OPSKEEP_API_KEY=<API_KEY>
   OPSKEEP_EMAIL=<EMAIL>
   OPSKEEP_USER_TIMEZONE=<IANA_TIMEZONE>
   ```
   ...
   ```text
   OPSKEEP_API_BASE_URL=https://pailflow-chat-gateway.fly.dev
   OPSKEEP_FOLLOW_UP_API_KEY=<API_KEY>
   OPSKEEP_FOLLOW_UP_EMAIL=<EMAIL>
   OPSKEEP_USER_TIMEZONE=<IANA_TIMEZONE>
   ```
   ...
   If `OPSKEEP_API_BASE_URL` and API key exist, continue to selected mode. If mode needs local-day interpretation and timezone missing, step 3.

3. **Set up missing creds only when needed.** Fill smallest missing piece.

   - Missing `OPSKEEP_USER_TIMEZONE` + local dates/vague times -> ask exactly: `What timezone should I use?` Stop.
   - Missing `OPSKEEP_API_BASE_URL` -> default `https://pailflow-chat-gateway.fly.dev` unless user named another gateway.
   - Missing `OPSKEEP_API_KEY` and `OPSKEEP_FOLLOW_UP_API_KEY` -> ask for user's email.

   Email-code setup:

   1. Explain email-code verification creates/replaces Opskeep agent utilities key. Mention key replacement revokes prior keys only if user may already have one.
   ...
   6. Prefer repo-local `.opskeep/credentials` for repo-local skills; `~/.opskeep/credentials` for global skills.
   7. Before repo-local write, ensure `.opskeep/` gitignored. Use file mode `600`.
...
4. **API contract.** Use only:
   ...
   ```text
   Authorization: Bearer ${OPSKEEP_API_KEY}
   ```
...
9. **Backfill mode.**
   ...
   1. Resolve date/time range with `OPSKEEP_USER_TIMEZONE`.
...
10. **Review mode.**
    ...
    1. Resolve range to concrete `started_at_gte` and `started_at_lt` UTC ISO. For "today", use `OPSKEEP_USER_TIMEZONE`.
...
12. **Top-up mode.**
    ...
    Time Tracking costs 10 Opskeep credits for 30 days. No auto-renewal; next mutating use after expiry charges another access pass.
    ...
    3. No amount -> ask exactly: `How many Opskeep credits do you want to buy?` Stop.
