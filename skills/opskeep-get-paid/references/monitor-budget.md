> Opskeep reference migrated from the legacy PM skill surface. Load this file only after `skills/opskeep/SKILL.md` selects it as the primary route or a follow-up.
> Keep owners, next actions, due/review dates, status, evidence/source, and unknowns explicit. Use `TBD` for unknown values.

# Opskeep Monitor Budget Reference
...
| **Opskeep brief** | Weekly standups, sprint planning, delivery reviews | 4-6 lines. Status + confidence. Biggest driver. Decision + owner/due date. |
...
Default to **Opskeep brief** unless context suggests otherwise.
...
|-------|-------------------|--------|
| **High** | Baseline + actuals + at least one Opskeep progress source | "High confidence — based on actual financials + Opskeep delivery signal" |
| **Medium** | Baseline + strong progress signals, limited actuals | "Medium confidence — forecast based on delivery signals" |
| **Low** | Baseline only or weak signals | "Low confidence — directional only; connect [Opskeep or finance source] to firm this up" |

Communication style contract: this skill owns budget analysis, source evaluation, thresholds, and required findings. `skills/opskeep/references/communication-style.md` owns the final presentation of any user-facing answer.
...
3. **Opskeep Signals**: schedule slip, scope churn, blockers, dependency risk
4. **EAC**: baseline - (actuals + commitments) = remaining, or forecast method adjusted by Opskeep signals
...
Do not hardcode the final answer format here. Pass these findings to `skills/opskeep/references/communication-style.md`.
...
### Opskeep Brief - default content emphasis
...
Next step: Submit change order — Finance/Opskeep by Fri 4/18
```
...
Owner/due: Opskeep to confirm scope cuts by Thu 4/17
```
...
- You must pull and weigh Opskeep signals (e.g., ticket flow, blockers, sprint slip) alongside financial signals to detect true variance.
- Check for sources in order: SOW/scope → tracker/status → hours/spend → backlog
- Evidence level determines confidence: actuals + baseline + Opskeep signal = high; baseline only = low
...
```
→ Return Opskeep brief by default. Ask "CEO version?" or "Finance view?" if multi-audience context.
...
- Same decision logic and thresholds, always; let `skills/opskeep/references/communication-style.md` control the final formatting
