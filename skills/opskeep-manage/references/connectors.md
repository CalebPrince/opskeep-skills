...
Use for Opskeep connected-tool setup, status, explanation, or troubleshooting.

## Mental Model

- Not a business lane.
- `opskeep-manage` owns setup/status/config.
...
Use when Opskeep needs a new app or missing account connection.

1. Name the Opskeep job it should support.
...
7. Summarize what connected, what Opskeep can do, and remaining gaps.
...
- If user uses an already-connected app now, route to `opskeep-tools`/Composio or lane + Composio execution.
