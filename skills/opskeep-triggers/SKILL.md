...
Create, inspect, and delete event-triggered automations for Opskeep.
...
- `event_type` should be normalized and human-readable to Opskeep, such as `pull_request.opened` or `issue.created`.
...
- Do not set `external_user_id`; the gateway uses the canonical Opskeep owner user ID as the Composio user key.
...
- `OPSKEEP_API_BASE_URL` - Base URL for the automation gateway, such as `https://api.opskeep.io` or `http://localhost:4110` in local mode
- `OPSKEEP_EXECUTION_SECRET` - Bearer token for authenticating API calls to `/v1/trigger-subscriptions`
- `OPSKEEP_ACCOUNT_ID` - Resolved account ID for the current Slack runtime when available
- `OPSKEEP_ACCOUNT_NAME` - Human-readable account name when available
- `OPSKEEP_CREATOR_USER_ID` - Internal creator user ID when available
- `OPSKEEP_REQUESTER_SLACK_USER_ID` - Slack user ID of the requester when available
- `OPSKEEP_REQUESTER_SLACK_TEAM_ID` - Slack workspace/team ID for the current runtime when available
...
```text
Authorization: Bearer ${OPSKEEP_EXECUTION_SECRET}
```

**Derived from request context:**

- `account_id` - Prefer `OPSKEEP_ACCOUNT_ID` when present instead of asking the user
- `creator_user_id` - Prefer `OPSKEEP_CREATOR_USER_ID` when present
- `requester_slack_user_id` - Prefer `OPSKEEP_REQUESTER_SLACK_USER_ID` when present
- `requester_slack_team_id` - Prefer `OPSKEEP_REQUESTER_SLACK_TEAM_ID` when present

Never hardcode credentials in this skill.

## Execution Contract

When this skill is running inside the Opskeep Slack sandbox, assume you can call the trigger subscription API directly if these environment variables are present:

- `OPSKEEP_API_BASE_URL`
- `OPSKEEP_EXECUTION_SECRET`
...
If `OPSKEEP_ACCOUNT_ID` is present, do **not** ask the user for an account ID. Use the injected value.
If `OPSKEEP_REQUESTER_SLACK_USER_ID` is present, do **not** ask the user for their Slack user ID. Use the injected value.
If `OPSKEEP_REQUESTER_SLACK_TEAM_ID` is present, do **not** ask the user for their Slack team/workspace ID. Use the injected value.

Preferred path:

1. Check that `OPSKEEP_API_BASE_URL` and `OPSKEEP_EXECUTION_SECRET` are present.
2. If `OPSKEEP_ACCOUNT_ID` is present, use it as `account_id`.
...
```bash
curl -sS -X POST "$OPSKEEP_API_BASE_URL/v1/trigger-subscriptions" \
  -H "Authorization: Bearer $OPSKEEP_EXECUTION_SECRET" \
...
```bash
curl -sS "$OPSKEEP_API_BASE_URL/v1/trigger-types?toolkit=<toolkit>" \
  -H "Authorization: Bearer $OPSKEEP_EXECUTION_SECRET"
```
...
```bash
curl -sS "$OPSKEEP_API_BASE_URL/v1/trigger-types/<provider_trigger_slug>" \
  -H "Authorization: Bearer $OPSKEEP_EXECUTION_SECRET"
```
...
```bash
curl -sS "$OPSKEEP_API_BASE_URL/v1/trigger-subscriptions?account_id=<account_id>" \
  -H "Authorization: Bearer $OPSKEEP_EXECUTION_SECRET"
```
...
```bash
curl -sS -X DELETE "$OPSKEEP_API_BASE_URL/v1/trigger-subscriptions/<id>?account_id=<account_id>" \
  -H "Authorization: Bearer $OPSKEEP_EXECUTION_SECRET"
```
...
- Do not ask the user for `account_id` if `OPSKEEP_ACCOUNT_ID` is already available.
- Do not ask the user for `requester_slack_user_id` if `OPSKEEP_REQUESTER_SLACK_USER_ID` is already available.
- Do not ask the user for `requester_slack_team_id` if `OPSKEEP_REQUESTER_SLACK_TEAM_ID` is already available.
