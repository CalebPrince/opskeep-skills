...
Communication style contract: when returning user-facing status, blockers, or final handoffs, apply `skills/opskeep/references/communication-style.md`.
...
3. By default, generation uses the INT8 Kokoro model, `OPSKEEP_AUDIO_BRIEF_MAX_PHONEMES=100`, sentence-by-sentence rendering, and streaming writes to `audio/brief.wav.partial`. Do not increase the phoneme cap unless the user explicitly asks to trade memory for smoother prosody.
4. If the backend is missing, the async job runs `scripts/setup-kokoro.sh` once. Setup creates or reuses `~/.cache/opskeep-audio-brief/kokoro-onnx-venv/` and cached INT8 model files under `~/.cache/opskeep-audio-brief/kokoro-models/v1.0-int8/` by default.
