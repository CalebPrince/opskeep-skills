...
CACHE_ROOT="${OPSKEEP_AUDIO_BRIEF_CACHE:-$HOME/.cache/opskeep-audio-brief}"
JOB_ROOT="${OPSKEEP_AUDIO_BRIEF_JOB_DIR:-$CACHE_ROOT/jobs}"

usage() {
  printf 'usage: %s start <brief-script.txt> <output.wav> [voice]\n' "$0" >&2
  printf '       OPSKEEP_AUDIO_BRIEF_BACKEND=kokoro|piper %s start <brief-script.txt> <output.wav> [voice-or-speaker]\n' "$0" >&2
...
run_generation() {
  ...
  local backend="${OPSKEEP_AUDIO_BRIEF_BACKEND:-kokoro}"
  local voice="${3:-af_heart}"
  local venv_dir="${OPSKEEP_AUDIO_BRIEF_KOKORO_VENV:-$CACHE_ROOT/kokoro-onnx-venv}"
  local model_variant="${OPSKEEP_AUDIO_BRIEF_MODEL_VARIANT:-int8}"
  local model_dir="${OPSKEEP_AUDIO_BRIEF_MODEL_DIR:-$CACHE_ROOT/kokoro-models/v1.0-$model_variant}"
  local model_file="${OPSKEEP_AUDIO_BRIEF_MODEL_FILE:-$model_dir/kokoro-v1.0.$model_variant.onnx}"
  if [[ "$model_variant" == "fp32" || "$model_variant" == "full" ]]; then
    model_file="${OPSKEEP_AUDIO_BRIEF_MODEL_FILE:-$model_dir/kokoro-v1.0.onnx}"
  fi
  local voices_file="$model_dir/voices-v1.0.bin"
  local max_phonemes="${OPSKEEP_AUDIO_BRIEF_MAX_PHONEMES:-100}"
  local max_words="${OPSKEEP_AUDIO_BRIEF_MAX_WORDS:-350}"
  local generation_timeout_seconds="${OPSKEEP_AUDIO_BRIEF_GENERATION_TIMEOUT_SECONDS:-1200}"
...
PY
  ...
  case "$backend" in
    kokoro)
      ;;
    piper)
      local piper_venv_dir="${OPSKEEP_AUDIO_BRIEF_PIPER_VENV:-$CACHE_ROOT/piper-venv}"
      local piper_model_name="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_NAME:-en_US-libritts_r-medium}"
      local piper_model_dir="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_DIR:-$CACHE_ROOT/piper-models/$piper_model_name}"
      local piper_model_file="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_FILE:-$piper_model_dir/$piper_model_name.onnx}"
      local piper_config_file="${OPSKEEP_AUDIO_BRIEF_PIPER_CONFIG_FILE:-$piper_model_dir/$piper_model_name.onnx.json}"
      local piper_bin="$piper_venv_dir/bin/piper"
      local piper_speaker="${OPSKEEP_AUDIO_BRIEF_PIPER_SPEAKER:-${3:-}}"
...
PY
    ...
    *)
      printf 'audio_result.status=blocked\n' >&2
      printf 'audio_result.reason=unsupported OPSKEEP_AUDIO_BRIEF_BACKEND: %s\n' "$backend" >&2
      exit 1
      ;;
  esac
...
wait_job() {
  ...
  local timeout_seconds="${3:-${OPSKEEP_AUDIO_BRIEF_WAIT_TIMEOUT_SECONDS:-900}}"
  local started_at
  local now
  local elapsed
  local status
