...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/opskeep-audio-brief-job-test.XXXXXX")"
trap 'rm -rf "$TMP_ROOT"' EXIT

export OPSKEEP_AUDIO_BRIEF_JOB_DIR="$TMP_ROOT/jobs"
mkdir -p "$OPSKEEP_AUDIO_BRIEF_JOB_DIR"
...
make_job() {
  ...
  local job_dir="$OPSKEEP_AUDIO_BRIEF_JOB_DIR/$job_id"
...
test_status_reconciles_failed_exit_race() {
  ...
  printf 'exit_code=1\n' > "$OPSKEEP_AUDIO_BRIEF_JOB_DIR/dead-failed/exit_code"
  printf 'audio_result.reason=kokoro setup failed\n' > "$OPSKEEP_AUDIO_BRIEF_JOB_DIR/dead-failed/stderr.log"
...
test_status_reconciles_successful_exit_race() {
  ...
  printf 'exit_code=0\n' > "$OPSKEEP_AUDIO_BRIEF_JOB_DIR/dead-ready/exit_code"
...
test_status_reports_ready_audio_metadata() {
  ...
  } > "$OPSKEEP_AUDIO_BRIEF_JOB_DIR/ready-metadata/stdout.log"
