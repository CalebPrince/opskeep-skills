...
VENV_DIR="${OPSKEEP_AUDIO_BRIEF_PIPER_VENV:-$CACHE_ROOT/piper-venv}"
MODEL_NAME="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_NAME:-en_US-libritts_r-medium}"
MODEL_DIR="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_DIR:-$CACHE_ROOT/piper-models/$MODEL_NAME}"
MODEL_FILE="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_FILE:-$MODEL_DIR/$MODEL_NAME.onnx}"
CONFIG_FILE="${OPSKEEP_AUDIO_BRIEF_PIPER_CONFIG_FILE:-$MODEL_DIR/$MODEL_NAME.onnx.json}"
PIPER_TTS_VERSION="${OPSKEEP_AUDIO_BRIEF_PIPER_TTS_VERSION:-1.4.2}"
...
MODEL_URL="${OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_URL:-$DEFAULT_MODEL_URL}"
CONFIG_URL="${OPSKEEP_AUDIO_BRIEF_PIPER_CONFIG_URL:-$DEFAULT_CONFIG_URL}"
...
create_venv() {
  if [[ -x "$VENV_DIR/bin/python" ]]; then
    ...
    if [[ -n "${OPSKEEP_AUDIO_BRIEF_PIPER_VENV:-}" ]]; then
      log "setup_result.status=blocked"
      log "setup_result.reason=OPSKEEP_AUDIO_BRIEF_PIPER_VENV points to an unsupported Python. Use Python 3.10-3.13, preferably Python 3.12."
      exit 1
    fi
