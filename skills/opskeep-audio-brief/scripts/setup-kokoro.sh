...
VENV_DIR="${OPSKEEP_AUDIO_BRIEF_KOKORO_VENV:-$CACHE_ROOT/kokoro-onnx-venv}"
MODEL_VARIANT="${OPSKEEP_AUDIO_BRIEF_MODEL_VARIANT:-int8}"
MODEL_DIR="${OPSKEEP_AUDIO_BRIEF_MODEL_DIR:-$CACHE_ROOT/kokoro-models/v1.0-$MODEL_VARIANT}"
MODEL_FILE="${OPSKEEP_AUDIO_BRIEF_MODEL_FILE:-$MODEL_DIR/kokoro-v1.0.$MODEL_VARIANT.onnx}"
VOICES_FILE="$MODEL_DIR/voices-v1.0.bin"
KOKORO_ONNX_VERSION="${OPSKEEP_AUDIO_BRIEF_KOKORO_ONNX_VERSION:-0.5.0}"
SOUNDFILE_VERSION="${OPSKEEP_AUDIO_BRIEF_SOUNDFILE_VERSION:-0.13.1}"
...
case "$MODEL_VARIANT" in
  ...
  *)
    ...
    printf 'setup_result.reason=unsupported OPSKEEP_AUDIO_BRIEF_MODEL_VARIANT: %s\n' "$MODEL_VARIANT" >&2
    exit 1
    ;;
esac

MODEL_URL="${OPSKEEP_AUDIO_BRIEF_MODEL_URL:-$DEFAULT_MODEL_URL}"
VOICES_URL="${OPSKEEP_AUDIO_BRIEF_VOICES_URL:-https://github.com/thewh1teagle/kokoro-onnx/releases/download/model-files-v1.0/voices-v1.0.bin}"
...
create_venv() {
  if [[ -x "$VENV_DIR/bin/python" ]]; then
    ...
    if [[ -n "${OPSKEEP_AUDIO_BRIEF_KOKORO_VENV:-}" ]]; then
      log "setup_result.status=blocked"
      log "setup_result.reason=OPSKEEP_AUDIO_BRIEF_KOKORO_VENV points to an unsupported Python. Use Python 3.10-3.13, preferably Python 3.12."
      exit 1
    fi
