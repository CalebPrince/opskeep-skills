...
Prefer Piper when the user asks for local generated audio without naming a backend. Use the cached `piper-tts` backend in `~/.cache/opskeep-audio-brief/` with `en_US-libritts_r-medium` by default.
...
2. Run `OPSKEEP_AUDIO_BRIEF_BACKEND=piper scripts/generate-audio-job.sh start <brief-script.txt> <publish-dir>/audio/brief.wav`.
...
Use Kokoro when the user specifically asks for Kokoro or when Piper is not desired: cached `kokoro-onnx` in `~/.cache/opskeep-audio-brief/`.
...
```bash
skills/opskeep-audio-brief/scripts/generate-audio-job.sh start brief-script.txt publish/audio/brief.wav
OPSKEEP_AUDIO_BRIEF_BACKEND=piper skills/opskeep-audio-brief/scripts/generate-audio-job.sh start brief-script.txt publish/audio/brief.wav
skills/opskeep-audio-brief/scripts/generate-audio-job.sh wait <job-id>
```

The async job:

- uses Piper `en_US-libritts_r-medium` when `OPSKEEP_AUDIO_BRIEF_BACKEND=piper`
- uses Kokoro `af_heart` by default unless a third voice argument is provided
- validates script word count before setup or generation and blocks default briefs over `OPSKEEP_AUDIO_BRIEF_MAX_WORDS=350`
- creates the cached backend if needed
- wraps generation with `OPSKEEP_AUDIO_BRIEF_GENERATION_TIMEOUT_SECONDS=1200` when `timeout` or `gtimeout` is available
...
For Kokoro only, the job also uses the INT8 Kokoro ONNX model by default, defaults to `OPSKEEP_AUDIO_BRIEF_MAX_PHONEMES=100`, and renders punctuation-delimited text chunks one at a time. Keep `OPSKEEP_AUDIO_BRIEF_MAX_PHONEMES=100` as the default. Higher values can smooth prosody but increase ONNX Runtime workspace memory.

For Piper, optional model overrides are available through `OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_NAME`, `OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_DIR`, `OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_FILE`, `OPSKEEP_AUDIO_BRIEF_PIPER_CONFIG_FILE`, `OPSKEEP_AUDIO_BRIEF_PIPER_MODEL_URL`, and `OPSKEEP_AUDIO_BRIEF_PIPER_CONFIG_URL`. Do not change these during normal skill use unless the user explicitly asks for another Piper voice.
...
skills/opskeep-audio-brief/scripts/generate-audio-job.sh start brief-script.txt publish/audio/brief.wav
skills/opskeep-audio-brief/scripts/generate-audio-job.sh wait <job-id>
skills/opskeep-audio-brief/scripts/generate-audio-job.sh status <job-id>
```

The job wrapper starts a detached generation process with `nohup`, stores job state under `~/.cache/opskeep-audio-brief/jobs/` by default, and returns immediately with an `audio_job.job_id`. Prefer `wait` for normal runs because it polls and emits progress until the job reaches a terminal state. `wait` accepts optional `poll-seconds` and `timeout-seconds` arguments, and also respects `OPSKEEP_AUDIO_BRIEF_WAIT_TIMEOUT_SECONDS` with a default of 900 seconds. Use `status` for debugging, progress checks, or when the caller's command timeout is shorter than the expected generation time.
...
- **Next:** run `skills/opskeep-audio-brief/scripts/setup-piper.sh` or `skills/opskeep-audio-brief/scripts/setup-kokoro.sh` after installing `uv` or Python 3.10-3.13, then retry generation
```
