...
- per-run job directories under `~/.cache/opskeep-audio-brief/jobs/` after here.now publish succeeds
- `.herenow/` or `.herenow/state.json` when created by this one-off publish run and not present before the run

Do not delete the managed Piper backend cache at `~/.cache/opskeep-audio-brief/piper-venv/` or `~/.cache/opskeep-audio-brief/piper-models/` after a successful run. Do not delete the managed Kokoro backend cache at `~/.cache/opskeep-audio-brief/kokoro-onnx-venv/` or `~/.cache/opskeep-audio-brief/kokoro-models/` after a successful run. Those caches are reusable setup that make future audio briefs fast when the environment persists them.
