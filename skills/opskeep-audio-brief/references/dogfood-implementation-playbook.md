...
4. Start audio with `scripts/generate-audio-job.sh`, passing absolute or repo-root-relative artifact paths. Let that job create the cached Piper or Kokoro backend if needed.
...
## Preferred Managed Generated-Audio Path

Use this async path for normal audio generation:

```bash
RUN_DIR=".artifacts/audio-briefs/<slug>"
skills/opskeep-audio-brief/scripts/generate-audio-job.sh start "$RUN_DIR/work/brief-script.txt" "$RUN_DIR/publish/audio/brief.wav"
skills/opskeep-audio-brief/scripts/generate-audio-job.sh wait <job-id>
```

If the command runner times out before `wait` completes, resume with `status <job-id>` rather than inspecting file size. A `.partial` output means generated audio is still writing or failed before final validation.

If the cached backend is missing, the job runs setup through the underlying generation script:

```bash
skills/opskeep-audio-brief/scripts/setup-piper.sh
skills/opskeep-audio-brief/scripts/setup-kokoro.sh
```

Setup uses `uv` with Python 3.12 when available, otherwise the first available supported Python 3.10-3.13 executable. If neither exists, block and ask for one of those prerequisites. Do not use Python 3.14 for local generated audio, and do not perform ad hoc package installs during a normal brief request.
...
If suspicious, do not publish. Treat it as blocked at generated audio completeness.
...
```bash
skills/opskeep-audio-brief/scripts/publish.sh <publish-dir> --client opencode
```
