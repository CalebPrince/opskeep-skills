...
- Reframe old `close`/`lessons`/`archive`/`handover` as public `improve operations`.
...
- `skills/opskeep-deliver-work/references/monitor-status.md`: delivery-state evidence.

## Connected Capabilities

- `opskeep-audio-brief`: only listenable/audio review.
- `composio`: only live source retrieval or external doc updates.
- `opskeep-triggers`: only recurring/event-driven learning capture.
...
- `examples/improve-operations.md`: output examples for lessons, handover, and reuse.
